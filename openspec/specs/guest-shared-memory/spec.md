...
Define the shared memory region lifecycle for Selium guests, comprising the `RegionProvider` trait that abstracts allocation, attach, and free operations behind a common interface, and the underlying hostcall ABI (`alloc_region`, `free_region`, `attach_region`) that the hostcall-backed implementation delegates to.
...
- **WHEN** a guest calls `provider.free` on a region with multiple attached instances
