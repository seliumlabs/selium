...
The `selium-rpc` crate SHALL provide an `RpcClient<Req, Rep>` that sends typed requests and receives typed replies over shared-memory ring buffers.

#### Scenario: Client sends request and receives reply
- **WHEN** a client calls `RpcClient::request(req)` with a valid request payload
- **THEN** the payload SHALL be rkyv-encoded and written as a frame to the request ring buffer, and the client SHALL block on the reply ring buffer's generation counter until a reply frame is ready
...
The `selium-rpc` crate SHALL provide an `RpcConnection<Req, Rep>` that receives typed requests and allows sending typed replies.
...
- **THEN** the server SHALL decode the request and return an `RpcRequest` that can call `.reply(response)` to write the reply to the reply ring
...
The `selium-rpc` crate SHALL establish RPC connections by having the client allocate a shared region, send its `shared_id` through a `HostQueue`, and both sides attach to that region.
