...
**Reason**: The `selium-quic` crate (and its `QuicTransport` `MessageTransport` impl) is deleted as part of the QUIC connector change; QUIC stream handling is now internal to the connector, which relays raw byte streams rather than typed `MessageTransport` framing.
**Migration**: Terminate QUIC at the edge with the `selium-connector-quic` connector and consume relayed byte channels via `selium-guest::net::quic`; frame application payloads with user-defined schemas over those channels.
