...
const SCHEMAS: [&str; 2] = ["schemas/atlas.fbs", "schemas/uri.fbs"];
...
fn rewrite_module_root(root: impl AsRef<Path>) -> std::io::Result<()> {
    ...
    let mut namespaces = list_dirs(&atlas_dir)?;
    namespaces.sort();

    let mut content =
        String::from("// Automatically generated by build.rs. Do not modify manually.\n");
    content.push_str("// Combined module tree for Atlas Flatbuffers namespaces.\n");
    content.push_str("pub mod atlas {\n");
    content.push_str("  use super::*;\n");

    for ns in namespaces {
        let dir = atlas_dir.join(&ns);
